...
3. ニックネームを入力（例: 島田）
4. 背番号を入力（任意）
5. チーム共通パスワードを入力: `futsal2024`
6. 「ログイン」をクリック

### 個人アカウントログイン

- パスワードログイン（既存ユーザー）
- メールリンクログイン（Magic Link）
...
3. **カレンダービュー** ✨
   - 月表示・週表示・日表示対応
   - イベントを視覚的に確認
   - 自分の回答ステータスで色分け（緑=参加、赤=不参加、黄=未定）
   - リストビューとの切り替え機能
...
## 最新のアップデート（2026-02-14）🎉

### UX/UI改善
1. **ボタンインタラクション強化**
   - 全ボタンにアクティブ状態のアニメーション追加（押下時に縮小＋透明化）
   - ローディング中のボタンを無効化し、「更新中...」「登録中...」などの状態表示

2. **不参加メンバーの可視化** 🆕
   - 出欠確認セクションに「参加」「不参加」「未定」の3つのリスト表示
   - ステータス別に色分け（緑・赤・グレー）
   - 各ユーザーのコメントも表示

3. **アプリタイトルのグローバル反映** 🆕
   - 設定ページで変更したアプリタイトルが全画面のヘッダーに即座に反映
   - チーム名やプロジェクト名に合わせてカスタマイズ可能

4. **背番号が任意項目に** 🆕
   - ユーザー登録時に背番号の入力は任意（未入力でも登録可能）
   - より柔軟なユーザー管理
